...
<main>
  <div class="hero">
    ...
    <div class="controls">
      ...
      <button id="clear" class="ghost">清除备用密钥</button>
    </div>
  </div>
  ...
  <section class="section" style="margin-top:8px"><h2><span>模型排行</span><span class="mini">仅 Codex 账号池</span></h2><div class="scroll model-table-wrap"><table><thead><tr><th>模型</th><th>别名</th><th>Provider</th><th>请求</th><th>总 Token</th><th>费用</th><th>输入</th><th>输出</th><th>缓存</th><th>缓存率</th></tr></thead><tbody id="models"></tbody></table></div></section>
  <section class="section" style="margin-top:8px"><h2><span>账号池运营台</span><span class="mini" id="account-scope">搜索、排序、分页承载大量账号</span></h2><div class="section-body"><div class="account-toolbar"><input id="account-filter" aria-label="搜索账号、邮箱或 AuthIndex" placeholder="搜索账号、邮箱或 AuthIndex"><select id="account-sort" aria-label="账号排序方式"><option value="tokens">按 Token</option><option value="cost">按费用</option><option value="quotaRemain">按 7d/月余量</option><option value="quotaTotal">按 7d/月总额度</option><option value="invalid">按 401 失效</option><option value="external">按外部消耗</option><option value="trigger">按触发状态</option><option value="quota">按额度已用</option><option value="cache">按缓存率</option><option value="429">按 429</option><option value="success">按成功率</option><option value="recent">按最近使用</option></select><span class="spacer"></span><select id="account-page-size" aria-label="账号每页数量"><option value="25">25 / 页</option><option value="50">50 / 页</option><option value="100">100 / 页</option></select><button id="account-prev" class="ghost" aria-label="上一页账号">上一页</button><span id="account-page-label" class="page-label">1 / 1</span><button id="account-next" class="ghost" aria-label="下一页账号">下一页</button></div><div class="account-summary-grid"><div class="account-summary-card"><span>当前结果</span><b id="account-loaded">-</b></div><div class="account-summary-card"><span>费用合计</span><b id="account-cost-total">-</b></div><div class="account-summary-card"><span>风险账号</span><b id="account-risk">-</b></div><div class="account-summary-card"><span>401 失效</span><b id="account-invalid-auth">-</b></div><div class="account-summary-card"><span>疑似外部消耗</span><b id="account-external-use">-</b></div><div class="account-summary-card"><span>触发异常</span><b id="account-trigger-failed">-</b></div><div class="account-summary-card"><span>额度最高</span><b id="account-quota-hot">-</b></div><div class="account-summary-card"><span>缓存最低</span><b id="account-cache-low">-</b></div></div><div class="scroll account-table-wrap"><table class="account-table"><thead><tr><th>账号</th><th>请求</th><th>成功率</th><th>总 Token / 费用</th><th>缓存</th><th>5h 窗口</th><th>7d/月窗口 / 额度预估</th><th>429</th><th>最近</th><th>状态</th></tr></thead><tbody id="account-table"></tbody></table></div></div></section>
  <section class="section" style="margin-top:8px"><h2><span>429 自动禁用状态</span><span class="mini">Codex reset_at 自动恢复</span></h2><div class="scroll"><table><thead><tr><th>账号</th><th>AuthIndex</th><th>窗口</th><th>原因</th><th>封禁时间</th><th>解禁时间</th><th>剩余</th><th>5h</th><th>7d</th></tr></thead><tbody id="autobans"></tbody></table></div></section>
  ...
  <section data-page="providers">
    ...
    <div class="layout">
      <section class="section"><h2><span>Provider / 接入点总览</span><span class="mini">按 Provider 名称聚合，不进入 Codex 账号池</span></h2><div class="scroll"><table><thead><tr><th>Provider</th><th>请求</th><th>成功率</th><th>总 Token</th><th>费用</th><th>输入</th><th>输出</th><th>缓存</th><th>缓存率</th><th>账号数</th><th>模型数</th><th>429</th><th>最近</th></tr></thead><tbody id="providers"></tbody></table></div></section>
    ...
    <section class="section" style="margin-top:8px"><h2><span>模型排行</span><span class="mini">其他 AI Provider</span></h2><div class="scroll model-table-wrap"><table><thead><tr><th>模型</th><th>别名</th><th>Provider</th><th>请求</th><th>总 Token</th><th>费用</th><th>输入</th><th>输出</th><th>缓存</th><th>缓存率</th></tr></thead><tbody id="provider-models"></tbody></table></div></section>
