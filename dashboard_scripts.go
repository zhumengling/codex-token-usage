...
document.getElementById('account-sort').onchange=()=>{accountPage=1;renderAccounts()};
document.getElementById('account-page-size').onchange=(e)=>{accountPageSize=Number(e.target.value)||25;accountPage=1;renderAccounts()};
...
document.getElementById('clear').onclick=()=>{sessionStorage.removeItem('cpa_token_usage_key');keyEl.value='';keyEl.classList.remove('on');document.getElementById('status').textContent='已清除备用管理密钥；页面会继续优先使用插件资源接口自动加载。'};
...
function ensureProviderSelection(providers){
  const names=providers.map(p=>providerLabel(p.provider));
...
function providerPageHTML(name){
  ...
  return '<section data-page="'+id+'">'+
    ...
    '</div>'+
    '<section class="section" style="margin-top:8px"><h2><span>模型排行</span><span class="mini">'+esc(name)+'</span></h2><div class="scroll model-table-wrap"><table><thead><tr><th>模型</th><th>别名</th><th>Provider</th><th>请求</th><th>总 Token</th><th>费用</th><th>输入</th><th>输出</th><th>缓存</th><th>缓存率</th></tr></thead><tbody id="'+id+'-models"></tbody></table></div></section>'+
...
function throughput(r){const ms=Number(r.latency_ms||0); const out=Number(r.output_tokens||0); return ms>0&&out>0?Math.round(out/(ms/1000))+' t/s':'-'}
...
async function load(){
  ...
  const st=document.getElementById('status'); st.textContent='加载中...';
  try{
    lastData=await fetchSummary(win,key);
    renderAll();
    st.textContent='窗口：'+lastData.window+' · 数据库：'+lastData.db_path+' · 更新时间：'+lastData.generated_at+' · '+(lastData._source==='resource'?'已自动获取':'备用密钥');
...
async function fetchSummary(win,key){
  ...
  if(!res.ok){
    ...
    throw new Error('HTTP '+res.status+' '+body+(res.status===401?' | 备用管理密钥不正确，已清除临时保存值。':''));
  }
...
function renderProviderPage(data){
  ...
  renderRecent('provider-recent',data.provider_recent||[],'provider');
}
...
function renderSingleProviderPage(data,name){
  ...
  const recent=(data.provider_recent||[]).filter(r=>providerEquals(r,name));
...
function renderTrend(target,points){
  ...
  const path=(key,max)=>points.map((p,i)=>{const x=pad+(w-pad*2)*(points.length===1?0:i/(points.length-1)); const y=h-pad-(h-pad*2)*((p[key]||0)/max); return (i?'L':'M')+x.toFixed(1)+' '+y.toFixed(1)}).join(' ');
...
function renderAccountTable(rows,total){
  document.getElementById('account-table').innerHTML=rows.map(r=>'<tr>'+
    ...
    td(metricStack(pct(cacheRate(r)),compact(cacheTokens(r))),'num')+
    td(quotaCompact('5h',r.primary_used_percent,r.primary_window_tokens,r.primary_reset_at),'num')+
    td(quota7dCell(r),'num')+
    td('<span class="'+((r.rate_limited||0)>0?'danger':'ok')+'">'+fmt(r.rate_limited||0)+'</span>','num')+
    td(esc(r.last_seen||'-'))+td(accountStatus(r))+
  '</tr>').join('') || '<tr><td colspan="10" class="muted">没有匹配的账号。</td></tr>';
}
...
function triggerBadge(r){
  ...
  const detail=(r.quota_trigger_mode||'quota')+' · '+(r.quota_trigger_last_at||'-')+(r.quota_trigger_http_status?' · HTTP '+r.quota_trigger_http_status:'')+(r.quota_trigger_error?' · '+r.quota_trigger_error:'');
...
function renderProviders(rows,total){
  document.getElementById('providers').innerHTML=rows.map(r=>'<tr>'+
    ...
    td(meterCell(compact(r.total_tokens),ratio(r.total_tokens,total),'var(--blue)'),'num')+td(costCell(r),'num')+td(compact(r.input_tokens),'num')+td(compact(r.output_tokens),'num')+
    td(compact(cacheTokens(r)),'num')+td(pct(cacheRate(r)),'num')+td(fmt(r.accounts||0),'num')+td(fmt(r.models||0),'num')+
    td(fmt(r.rate_limited||0),'num')+td(esc(r.last_seen||'-'))+'</tr>').join('') || '<tr><td colspan="13" class="muted">暂无 Provider 数据</td></tr>';
}
...
function renderInsights(data){
  ...
  const items=[
    ...
    ['额度触发',qt.enabled?((qt.mode||'quota')+' · '+(qt.interval_minutes||10)+'m'):'已关闭',triggerLine,qt.last_failed?'tone-orange':qt.enabled?'tone-green':''],
...
function renderModels(target,rows){
  document.getElementById(target).innerHTML=rows.map(r=>'<tr>'+td(esc(r.model))+td(esc(r.alias))+td(esc(r.provider))+td(fmt(r.requests),'num')+td(compact(r.total_tokens),'num')+td(costCell(r),'num')+td(compact(r.input_tokens),'num')+td(compact(r.output_tokens),'num')+td(compact(cacheTokens(r)),'num')+td(pct(cacheRate(r)),'num')+'</tr>').join('') || '<tr><td colspan="10" class="muted">暂无模型数据</td></tr>';
}
